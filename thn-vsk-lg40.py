...
def traqt(i,soso,cq): 
    try: 
        ...
        cmnd=browser.find_element_by_name("soCmnd").get_attribute("value")
        qt=len(tr)
        if("smsApp.tongHopQt" in tgbhxh):
            browser.quit()
            login()
        if(qt>2):
        ...
        if("Timed out receiving message from renderer" in str(e)) or "Failed to execute script thn-vsk" in str(e) or "no such element: Unable to locate element" in str(e):
            browser.quit()
            login()
