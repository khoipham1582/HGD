...
def huygt(mh,ms):
    ...
    try:
        ...
        browser.execute_script("arguments[0].value='230288268'",socmnd)
        ...
        #if sdt.text!="": 
        browser.execute_script("arguments[0].value='0943663668'",sdt)
        #sdt.send_keys("0943653668")
        #suatt=browser.find_element_by_id("cmdAddNew")
        try:
            kttt=browser.find_element_by_id("cmdKiemTra")
            kttt.click()
            dnh=browser.find_element_by_id("cmdDeNghiDuyet")
            dnh.click()
            
        except:
            suatt=browser.find_element_by_id("cmdAddNew")
            suatt.click()
        #print("Đề nghị hủy GT")
...
for i in range(len(df.iloc[:,0])):
    ...
    huygt(str(df.at[i,0]),str(df.at[i,1]))
    xoacmnd()